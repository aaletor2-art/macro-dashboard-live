(() => {
  ...
  const countrySymbols = {"United Kingdom":"GBPUSD=X","United States":"DX-Y.NYB","Euro Area":"EURUSD=X",Bulgaria:"EURUSD=X",Japan:"JPY=X",China:"CNY=X",India:"INR=X","South Korea":"KRW=X",Taiwan:"TWD=X",Singapore:"SGD=X","Hong Kong":"HKD=X",Australia:"AUDUSD=X",Canada:"CAD=X",Brazil:"BRL=X",Mexico:"MXN=X",Nigeria:"NGN=X","South Africa":"ZAR=X",Egypt:"EGP=X",Kenya:"KES=X",Ghana:"GHS=X",Morocco:"MAD=X",Angola:"AOA=X",Ethiopia:"ETB=X","Cote d'Ivoire":"XOF=X",Rwanda:"RWF=X"};
  ...
  function countryPrice(row) {
    ...
    if(fallback) return { ...fallback, value:fallback.close, marketTime:fallback.date, current:false };
    if(symbol){const convention=symbol==="DX-Y.NYB"?"US Dollar Index points":symbol==="EURUSD=X"?"USD per 1 EUR":symbol==="GBPUSD=X"?"USD per 1 GBP":"USD per 1 local-currency unit";return{asset:`${row.market} market reference`,symbol,convention,value:null,marketTime:live.generatedAt||hosted.generatedAt,current:true,unavailable:true};}
    return null;
  }
  ...
  function countryNewsHtml(row) {
    ...
    return `<section class="country-news-panel"><div class="card-title-row"><div><span class="eyebrow">NEWS &amp; CATALYSTS</span><h3>${row.market} and global macro watch</h3></div><small>10-item rule · Reuters and Trading Economics discovery</small></div><div id="country-news-status" class="country-news-status">Loading the latest country-specific reporting…</div><ol id="country-news-list" class="country-news-list">${items.map(item=>newsListItem(item)).join("")}</ol></section>`;
  }

  const newsListItem=item=>`<li><a href="${item.url}" target="_blank" rel="noopener"><span>${item.source || "Source"}${item.publishedAt?` · ${new Date(item.publishedAt).toLocaleDateString("en-GB",{day:"2-digit",month:"short"})}`:""}</span><strong>${item.title}</strong><small>${item.explanation||whyItMatters(item.title)}</small></a></li>`;
  async function loadCountryNews(row){const list=document.getElementById("country-news-list"),status=document.getElementById("country-news-status");if(!list)return;const fallback=hosted.news||[],teSlug=countryCodes[row.market]||row.market.toLowerCase().replaceAll(" ","-");const discovery=[{source:"Reuters search",title:`Latest Reuters reporting on ${row.market}`,url:`https://www.reuters.com/site-search/?query=${sourceQuery(row.market)}`,explanation:`Fresh reporting and policy developments relevant to ${row.market}.`},{source:"Trading Economics",title:`Latest ${row.market} indicators and forecasts`,url:`https://tradingeconomics.com/${teSlug}/indicators`,explanation:`Current releases, forecasts and historical context for ${row.market}.`}];let items=[...fallback,...discovery].slice(0,10);const paint=()=>{list.innerHTML=items.map(newsListItem).join("");if(status)status.textContent=`Showing ${items.length} of 10 relevant links · checked ${new Date().toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}`;};paint();try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),5000);const response=await fetch(`/api/news?country=${encodeURIComponent(row.market)}&ts=${Date.now()}`,{cache:"no-store",signal:controller.signal});clearTimeout(timer);if(response.ok){const fresh=(await response.json()).items||[];items=[...fresh,...fallback.filter(item=>!fresh.some(found=>found.title===item.title)),...discovery.filter(item=>!fresh.some(found=>found.title===item.title))].slice(0,10);paint();}}catch{}}
  ...
  function renderCountry(name) {
    ...
    document.getElementById("country-profile").innerHTML = `
      ...
      <section class="country-live-price"><div><span class="eyebrow">CURRENT MARKET REFERENCE</span><h3>${quote ? `${quote.asset} · ${quote.value==null?"quote temporarily unavailable":number(quote.value,quote.value>100?2:5)}` : "No direct free FX quote mapped"}</h3><p>${quote ? `${quote.symbol} · ${quote.convention} · ${quote.current?"5-minute cache":"latest hosted fallback"}` : "Verify a tradable instrument on the markets page."}</p></div>${quote?`<div class="quote-meta"><span class="badge ${(quote.changePct||0)>0?"up":(quote.changePct||0)<0?"down":"flat"}">${quote.unavailable?"Feed unavailable":quote.changePct==null?"Latest":`${quote.changePct>=0?"+":""}${number(quote.changePct)}%`}</span><small>${new Date(quote.marketTime || live.generatedAt || hosted.generatedAt).toLocaleString("en-GB",{dateStyle:"medium",timeStyle:"short"})}</small></div>`:""}<a href="/markets">Open price tape →</a></section>
      ${recentBriefHtml(row)}
    ...
    renderComparison("country-comparison", [row.market,"United States","China"], "Policy rate");
    loadCountryNews(row);
  }
  ...
  function drawComparisonChart(canvas, names, indicator, fields) {
    ...
    const key=fields.find(([label])=>label===indicator)?.[1]||"cpi"; const rows=names.map(name=>{const mapped=templateIndicators.find(row=>row.market===name),market=markets.find(row=>row.market===name);const currentOnly=key==="policy_rate"&&market?market.rate:key==="real_rate"&&market?realRateFor(market):key==="cpi"&&market?market.inflation:null;const data=mapped?.[key]||(Number.isFinite(currentOnly)?{previous:null,current:currentOnly,forecast:null}:null);return data?{market:name,data}:null}).filter(Boolean); const series=["previous","current","forecast"], colours=["#9aa49f","#087a55","#375a7f"];
  ...
  function renderCompanies() {
    ...
    if(!countryFilter.dataset.bound&&companyUniverse.length){const requested=new URLSearchParams(location.search).get("country")||"All";countryFilter.innerHTML=`<option value="All" ${requested==="All"?"selected":""}>🌍 All countries · ${companyUniverse.length} companies</option>`+[...new Set(companyUniverse.map(x=>x.country))].map(country=>`<option value="${country}" ${country===requested?"selected":""}>${flags[country]||"🌍"} ${country}</option>`).join("");countryFilter.addEventListener("change",loadCompanyCountry);countryFilter.dataset.bound="true";loadCompanyCountry();}
    ...
    const rows = companyQuotes.filter(row => (companySector === "All" || row.sector === companySector)&&(!search||`${row.name} ${row.symbol||""} ${row.country} ${row.sector}`.toLowerCase().includes(search))).sort((a,b)=>sort==="move"?Math.abs(b.changePct||0)-Math.abs(a.changePct||0):sort==="name"?a.name.localeCompare(b.name):sort==="country"?a.country.localeCompare(b.country)||a.rank-b.rank:(a.rank||99)-(b.rank||99)||a.country.localeCompare(b.country));
    const count=document.getElementById("company-result-count");if(count)count.innerHTML=`Showing <strong>${rows.length}</strong> of ${companyQuotes.length} companies${countryFilter.value==="All"?" across the full list":` in ${countryFilter.value}`}`;
  ...
  async function loadCompanyCountry(){const select=document.getElementById("company-country-filter");if(!select)return;document.getElementById("company-grid").innerHTML=`<div class="empty-state">Loading company list…</div>`;if(select.value==="All"){companyQuotes=companyUniverse.map(x=>({...x,unavailable:true,reason:x.symbol?"Open profile for current quote":"No stable free exchange feed"}));companyView="compact";document.querySelectorAll("[data-company-view]").forEach(button=>button.classList.toggle("active",button.dataset.companyView==="compact"));document.getElementById("company-refresh").textContent=`Full universe · ${companyQuotes.length} companies`;renderCompanies();return;}try{const response=await fetch(`/api/companies?country=${encodeURIComponent(select.value)}&ts=${Date.now()}`);const data=await response.json();companyQuotes=data.peers||[];document.getElementById("company-refresh").textContent=`Updated ${new Date(data.generatedAt).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}`;}catch{companyQuotes=companyUniverse.filter(x=>x.country===select.value).slice(0,5).map(x=>({...x,unavailable:true}));}renderCompanies();}
  ...
  function fillTradeSheet() {
    ...
    const counter=markets.find(row=>row.market===decodeURIComponent(tradeValue("plan-counter-country")))||markets[1], rateDiff=country.rate==null||counter.rate==null?null:country.rate-counter.rate, realDiff=realRateFor(country)==null||realRateFor(counter)==null?null:realRateFor(country)-realRateFor(counter), headlines=planNews(country,counter), rateLeader=rateDiff==null?"neither side until rates are verified":rateDiff>=0?country.market:counter.market;
    document.getElementById("trade-entry").value=item?.price||"";
    document.getElementById("trade-macro-logic").value=`ASSET: ${item?.symbol||"Selected asset"} · ${direction.toUpperCase()}\n\nPRIMARY: ${country.market} — inflation ${pct(country.inflation)}, policy rate ${pct(country.rate)}, real rate ${pct(real)}, pressure ${country.pressure}/100 and bias ${country.bias}.\n\nCOMPARISON: ${counter.market} — inflation ${pct(counter.inflation)}, policy rate ${pct(counter.rate)}, real rate ${pct(realRateFor(counter))}, pressure ${counter.pressure}/100 and bias ${counter.bias}.\n\nRELATIVE EDGE: nominal rate differential ${rateDiff==null?"n/a":`${rateDiff>=0?"+":""}${number(rateDiff)} percentage points`}; real-rate differential ${realDiff==null?"n/a":`${realDiff>=0?"+":""}${number(realDiff)} percentage points`} in favour of ${rateLeader}. The setup is valid only if growth, inflation credibility, news flow and price action confirm the same direction.\n\nNEWS CHECK: ${headlines.map(item=>item.title).join(" | ")||"No hosted headlines; check both country news pages."}\n\nINVALIDATION: inflation, central-bank guidance, relative growth or the rate advantage reverses, or price fails the technical trigger.`;
    document.getElementById("trade-fundamental").value=`${interpretation(country)} Against ${counter.market}, the nominal policy-rate differential is ${rateDiff==null?"n/a":`${number(rateDiff)} percentage points`} and the real-rate differential is ${realDiff==null?"n/a":`${number(realDiff)} percentage points`}. Primary catalysts: ${country.watch}. Counter-country catalysts: ${counter.watch}. Review the economic calendar and both news lists before entry.`;
  ...
  function assetCountryPair(symbol){const pairs={"GBPUSD=X":["United Kingdom","United States"],"EURUSD=X":["Euro Area","United States"],"JPY=X":["Japan","United States"],"AUDUSD=X":["Australia","United States"],"CAD=X":["Canada","United States"],"CNY=X":["China","United States"],"INR=X":["India","United States"],"KRW=X":["South Korea","United States"],"TWD=X":["Taiwan","United States"],"SGD=X":["Singapore","United States"],"HKD=X":["Hong Kong","United States"],"BRL=X":["Brazil","United States"],"MXN=X":["Mexico","United States"],"NGN=X":["Nigeria","United States"],"ZAR=X":["South Africa","United States"],"EGP=X":["Egypt","United States"],"KES=X":["Kenya","United States"],"GHS=X":["Ghana","United States"],"MAD=X":["Morocco","United States"],"AOA=X":["Angola","United States"],"ETB=X":["Ethiopia","United States"],"XOF=X":["Cote d'Ivoire","United States"],"RWF=X":["Rwanda","United States"]};return pairs[symbol]||["United States","Euro Area"];}

  function planNews(primary,counter){const terms=[primary.market,counter.market,...primary.watch.split(","),...counter.watch.split(",")].map(x=>x.trim().toLowerCase());const relevant=(hosted.news||[]).filter(item=>terms.some(term=>term.length>3&&item.title?.toLowerCase().includes(term)));return [...relevant,...(hosted.news||[]).filter(item=>!relevant.includes(item))].slice(0,5);}
  ...
  function renderTradingPlan() {
    ...
    const counterSelect = document.getElementById("plan-counter-country");
    if (!assetSelect || !countrySelect || !counterSelect) return;
    if (!assetSelect.dataset.bound) {
      ...
      counterSelect.innerHTML = markets.map(row => `<option value="${encodeURIComponent(row.market)}">${flags[row.market] || "🌍"} ${row.market}</option>`).join("");
      ...
      assetSelect.addEventListener("change",()=>{const [primary,counter]=assetCountryPair(assetSelect.value);countrySelect.value=encodeURIComponent(primary);counterSelect.value=encodeURIComponent(counter);renderTradingPlan();});
      [countrySelect,counterSelect,document.getElementById("plan-direction")].forEach(input => input.addEventListener("change", renderTradingPlan));
      document.querySelectorAll("[data-indicator]").forEach(input => input.addEventListener("change", renderTradingPlan));
      const [initialPrimary,initialCounter]=assetCountryPair(assetSelect.value);countrySelect.value=encodeURIComponent(initialPrimary);counterSelect.value=encodeURIComponent(initialCounter);
      ...
      const [primary,counter]=assetCountryPair(assetSelect.value);countrySelect.value=encodeURIComponent(primary);counterSelect.value=encodeURIComponent(counter);
    }
    ...
    const counterName=decodeURIComponent(counterSelect.value||"United States"), counter=markets.find(row=>row.market===counterName)||markets[1];
    ...
    const tiltFor=market=>{let score=0,marketReal=realRateFor(market);if(/hawk|hike|restrict/i.test(market.bias))score+=15;if(/easing|support|cool/i.test(`${market.bias} ${market.temp}`))score-=15;if(marketReal!=null)score+=marketReal>0?8:-8;if(market.inflation>5)score+=7;return score;};
    const macroTilt = tiltFor(country)-tiltFor(counter);
    ...
    const rateDiff=country.rate==null||counter.rate==null?null:country.rate-counter.rate, realDiff=realRateFor(country)==null||realRateFor(counter)==null?null:realRateFor(country)-realRateFor(counter), news=planNews(country,counter);
    document.getElementById("plan-relative-macro").innerHTML=`<section class="relative-rate-card"><span class="eyebrow">INTEREST-RATE COMPARISON</span><h3>${flags[country.market]||"🌍"} ${country.market} versus ${flags[counter.market]||"🌍"} ${counter.market}</h3><div class="rate-versus"><div class="rate-country"><span>${country.market}</span><strong>${pct(country.rate)}</strong><small>Real rate ${pct(realRateFor(country))} · ${country.bias}</small></div><div class="rate-differential"><strong>${rateDiff==null?"n/a":`${rateDiff>=0?"+":""}${number(rateDiff)} pp`}</strong><small>nominal differential<br>${realDiff==null?"n/a":`${realDiff>=0?"+":""}${number(realDiff)} pp real`}</small></div><div class="rate-country"><span>${counter.market}</span><strong>${pct(counter.rate)}</strong><small>Real rate ${pct(realRateFor(counter))} · ${counter.bias}</small></div></div><p class="rate-comparison-note">A positive differential can support the primary currency, but only when inflation credibility, growth and risk sentiment agree.</p></section><section class="plan-news-card"><span class="eyebrow">NEWS CHECK</span><h3>Latest themes affecting this pair</h3><ol class="plan-news-list">${news.map(item=>`<li><a href="${item.url}" target="_blank" rel="noopener"><span>${item.source||"Source"}</span>${item.title}</a></li>`).join("")||"<li>No hosted headlines available; check the country news pages before entry.</li>"}</ol></section>`;
    ...
    document.getElementById("fundamental-read").innerHTML=`<div class="read-cell"><span>INFLATION</span><strong>${pct(country.inflation)}</strong><small>${country.temp} regime</small></div><div class="read-cell"><span>POLICY RATE</span><strong>${pct(country.rate)}</strong><small>${country.bias}</small></div><div class="read-cell"><span>RATE DIFFERENTIAL</span><strong>${rateDiff==null?"n/a":`${rateDiff>=0?"+":""}${number(rateDiff)} pp`}</strong><small>Versus ${counter.market}</small></div><div class="read-cell"><span>REAL RATE</span><strong>${pct(real)}</strong><small>Real differential ${realDiff==null?"n/a":`${number(realDiff)} pp`}</small></div><div class="read-cell"><span>PRESSURE</span><strong>${country.pressure}/100</strong><small>${country.watch}</small></div><div class="read-cell"><span>DIRECTIONAL SCORE</span><strong>${fundamentalScore}/100</strong><small>${direction} rule result</small></div>`;
    const bb=item?.technical?.bollinger, stoch=item?.technical?.stochastic;
