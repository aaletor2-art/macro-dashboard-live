(() => {
  ...
  async function loadCountryNews(row){const list=document.getElementById("country-news-list"),status=document.getElementById("country-news-status");if(!list)return;let items=[];try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),5000);const response=await fetch(`/api/news?country=${encodeURIComponent(row.market)}&ts=${Date.now()}`,{cache:"no-store",signal:controller.signal});clearTimeout(timer);if(response.ok)items=(await response.json()).items||[];}catch{}const fallback=hosted.news||[];const discovery=[{source:"Reuters search",title:`Latest Reuters reporting on ${row.market}`,url:`https://www.reuters.com/site-search/?query=${sourceQuery(row.market)}`,explanation:`Fresh reporting and policy developments relevant to ${row.market}.`},{source:"Trading Economics",title:`Latest ${row.market} indicators and forecasts`,url:`https://tradingeconomics.com/${slug(row.market)}/indicators`,explanation:`Current releases, forecasts and historical context for ${row.market}.`}];items=[...items,...fallback.filter(item=>!items.some(found=>found.title===item.title)),...discovery.filter(item=>!items.some(found=>found.title===item.title))].slice(0,10);list.innerHTML=items.map(newsListItem).join("");if(status)status.textContent=`Showing ${items.length} of 10 latest relevant links · refreshed ${new Date().toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}`;}
