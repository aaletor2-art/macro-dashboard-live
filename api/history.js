...
const historicalIndicators = {
  gdp: { indicator: "NY.GDP.MKTP.KD.ZG", label: "Real GDP growth", unit: "% annual growth" },
  unemployment: { indicator: "SL.UEM.TOTL.ZS", label: "Unemployment rate", unit: "% of labour force" },
  gini: { indicator: "SI.POV.GINI", label: "Gini index", unit: "index points" }
};
...
export default async function handler(request, response) {
  ...
  const indicatorKey = String(request.query?.indicator || "inflation").toLowerCase();
  const country = countries[slug];
  if (!country) return response.status(404).json({ error: "Country not mapped" });
  if (indicatorKey !== "inflation") {
    const config = historicalIndicators[indicatorKey];
    if (!config) return response.status(400).json({ error: "Indicator not supported" });
    const observations = await annualIndicator(country.iso, config);
    return response.status(200).json({ country: country.name, indicatorKey, indicator: config.label, unit: config.unit, frequency: "annual", source: "World Bank", sourceUrl: `https://data.worldbank.org/indicator/${config.indicator}?locations=${country.iso}`, generatedAt: new Date().toISOString(), observations });
  }
  ...
  return response.status(200).json({ country: country.name, indicatorKey, indicator: "Consumer price inflation", unit: "% year on year", frequency, source, sourceUrl: source.startsWith("FRED") ? `https://fred.stlouisfed.org/series/${country.fred}` : `https://data.worldbank.org/indicator/FP.CPI.TOTL.ZG?locations=${country.iso}`, generatedAt: new Date().toISOString(), observations });
}
